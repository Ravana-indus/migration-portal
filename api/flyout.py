...
def log_sync_attempt(doctype, reference_id, operation, request_data, response_data=None, error=None):
    """
    Log a sync attempt with FlyOut.
    
    Args:
        doctype (str): The DocType being synced
        reference_id (str): The ID of the record in FlyOut 
        operation (str): The operation type (PUSH, PULL, etc.)
        request_data (dict): The data sent to FlyOut
        response_data (dict, optional): The response from FlyOut
        error (str, optional): Error message if any
    
    Returns:
        str: The name of the created log entry
    """
    try:
        # Create a simple log entry
        frappe.logger().info(f"FlyOut Sync: {doctype} {reference_id} {operation}")
        
        # In a real implementation, you would create a SyncLog document here
        # log = frappe.new_doc("Sync Log")
        # log.reference_doctype = doctype
        # log.flyout_reference_id = reference_id
        # log.operation = operation
        # log.request_data = json.dumps(request_data, indent=2)
        # if response_data:
        ...
        return f"{doctype}-{reference_id}-{now_datetime().strftime('%Y%m%d%H%M%S')}"
    except Exception as e:
        frappe.logger().error(f"Error logging sync attempt: {str(e)}")
        return None 